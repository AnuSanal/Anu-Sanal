...
inception<-NULL
url1<-"https://www.imdb.com/title/tt1375666/reviews?start=?"
 for(i in 0:22){
   url<-read_html(as.character(paste(url1,i*a,sep="")))
   incept<-url %>%
     html_nodes("#tn15content div+ p") %>%
     html_text() 
   inception<-c(inception,incept)
 }
 write.table(inception,file="inception.txt")
 View(inception)
 install.packages("syuzhet")
 library("syuzhet")
 
 my_example_text <- readLines("inception.txt")
